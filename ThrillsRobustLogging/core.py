...
class ThrillsRobustLogging(commands.GroupCog, group_name="logset"):
    ...
    async def _get_audit_log_entry(self, guild: discord.Guild, target: Union[discord.User, discord.Role, discord.abc.GuildChannel], action: discord.AuditLogAction) -> Optional[discord.AuditLogEntry]:
        """A centralized helper to fetch the latest audit log entry for a specific target."""
        await asyncio.sleep(0.5)
        try:
            # Asynchronously iterate through the audit log generator to find the entry
            async for entry in guild.audit_logs(limit=5, action=action):
                if entry.target and entry.target.id == target.id:
                    return entry  # Return the found entry
            return None # Return None if no entry was found
    ...
    async def category_autocomplete(self, interaction: discord.Interaction, current: str) -> List[app_commands.Choice[str]]:
        ...
        return [app_commands.Choice(name=cat.replace("_", " ").title(), value=cat) for cat in categories if current.lower() in cat.lower()]
    ...
    async def setting_autocomplete(self, interaction: discord.Interaction, current: str) -> List[app_commands.Choice[str]]:
        ...
        return [app_commands.Choice(name=setting.replace("_", " ").title(), value=setting) for setting in settings if current.lower() in setting.lower()]

    @app_commands.command(name="channel", description="Set a log channel for a specific category.")
    @app_commands.describe(category="The category of logs to set the channel for.", channel="The text channel to send logs to. Leave blank to reset.")
    @app_commands.choices(category=[app_commands.Choice(name=cat.replace("_", " ").title(), value=cat) for cat in DEFAULT_GUILD_SETTINGS["log_channels"].keys()])
    ...
    async def logset_status(self, interaction: discord.Interaction):
        ...
        channel_desc = [f"**{cat.title()}**: {interaction.guild.get_channel(cid).mention if cid else 'Not Set'}" for cat, cid in all_channels.items()]
        embed.add_field(name="Log Channels", value="\n".join(channel_desc), inline=False)
        for category, settings in all_toggles.items():
            toggle_desc = [f"{'✅' if enabled else '❌'} {setting.replace('_', ' ').title()}" for setting, enabled in settings.items()]
